...
def calculate_direct_risk_matrix(direct_likelihood_matrix, direct_impact_matrix):
    ...
    """
    return (
        np.array(direct_likelihood_matrix) * np.array(direct_impact_matrix)
    ).tolist()
...
def plot_propagation_tree(propagation_tree):
    ...
    nx.draw_networkx(propagation_tree, pos=pos, labels=labels, **options)
...
def calculate_combined_likelihood_matrix(
    design_structure_matrix, direct_likelihood_matrix, cutoff
):
    ...
    g = nx.from_numpy_array(
        np.transpose(np.matrix(design_structure_matrix)), create_using=nx.DiGraph
    )
    combined_likelihood_matrix = [
        [0 for col in range(number_elements)] for row in range(number_elements)
    ]
            ...
            if target != source:
                ...
                for node in nodes_list:
                    ...
                    if parents != []:
                        ...
                        if len(children) == 0:
                            tree.edges[parent, node]["likelihood"] = (
                                direct_likelihood_matrix[tree.nodes[node]["name"]][
                                    tree.nodes[parent]["name"]
                                ]
                            )
                        elif len(children) == 1:
                            child = children[0]
                            tree.edges[parent, node]["likelihood"] = tree.edges[
                                node, child
                            ]["likelihood"]
                        else:
                            temp = 1
                            for child in children:
                                temp = temp * (
                                    1 - tree.edges[node, child]["likelihood"]
                                )
...
def calculate_combined_risk_matrix(
        ...
        for source, element in enumerate(row):
            if target != source:
                ...
                for penultimate in penultimates:
                    temp = temp * (
                        1
                        - combined_likelihood_matrix[penultimate][source]
                        * direct_likelihood_matrix[target][penultimate]
                        * direct_impact_matrix[target][penultimate]
                    )
...
def calculate_combined_impact_matrix(
    design_structure_matrix, combined_likelihood_matrix, combined_risk_matrix
):
    ...
    for i, row in enumerate(combined_risk_matrix):
        for j, element in enumerate(row):
            if i != j and combined_likelihood_matrix[i][j] != 0:
                combined_impact_matrix[i][j] = (
                    combined_risk_matrix[i][j] / combined_likelihood_matrix[i][j]
                )
    return combined_impact_matrix


def calculate_all_matrices(
    design_structure_matrix, direct_likelihood_matrix, direct_impact_matrix, cutoff
):
...
def plot_product_risk_matrix(product_elements, DSM, clm, cim, crm):
    ...
    for x, y, c, h, w in zip(x, y, z, dx, dy):
        ax.add_artist(Rectangle(xy=(x, y), color=cmap(c), width=w, height=h))
    ...
    # Add a colorbar - **Adjustment Here**
